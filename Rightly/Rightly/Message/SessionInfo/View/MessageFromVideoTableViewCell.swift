//
//  MessageFromVideoTableViewCell.swift
//  Rightly
//
...
class MessageFromVideoTableViewCell: UITableViewCell {
    ...
    var  vmmodel:MessageVideoViewModel?=nil
    ...
    func bindingViewModel(_ viewModel:MessageVideoViewModel) {
        ...
        viewModel.receiveStatus.subscribe(onNext: { [weak self] status in
            ...
            self.progressView.isHidden = (status != .receiving)
            self.playBtn.isHidden = (status != .received)
