...
open class LVCycleScrollView: UIView, UICollectionViewDelegate, UICollectionViewDataSource {
    
    /// 外部传入的本地图片
    @objc public var localizationImageNameArray: [String]? {
        didSet {
            self.collectionView.register(LVUIImageViewCell.self, forCellWithReuseIdentifier: kImageViewCellId)
    ...
    @objc public var viewArray: [UIView]? {
        didSet {
            self.collectionView.register(LVUIviewCell.self, forCellWithReuseIdentifier: kViewCellId)
    ...
    open override func layoutSubviews() {
        ...
        if self.pageControl != nil {
            ...
            } else if self.pageControl!.isKind(of: LVPageControl.self) {
                var tmpW: CGFloat = 0.0
                var tmpH: CGFloat = 0.0
                if self.customDotViewType == .image && self.pageDotImage != nil {
                    tmpW = CGFloat(self.sourceCount) * self.pageDotImage!.size.width + ((self.pageControl as! LVPageControl).spacingBetweenDots * CGFloat((self.sourceCount - 1)))
                    tmpH = self.pageDotImage!.size.height
                } else {
                    tmpW = CGFloat(self.sourceCount) * self.pageControlDotSize.width + ((self.pageControl as! LVPageControl).spacingBetweenDots * CGFloat((self.sourceCount - 1)))
...
// MARK: - 初始化方法
extension LVCycleScrollView {
    
    /// 类初始化方法
    ///
    /// - Parameter frame: FWCycleScrollView的大小
    /// - Returns: self
    @objc open class func cycle(frame: CGRect) -> LVCycleScrollView {
        
        let cycleScrollView = LVCycleScrollView(frame: frame)
    ...
    /// - Returns: self
    @objc open class func cycle(frame: CGRect, loopTimes: Int) -> LVCycleScrollView {
    ...
    /// - Returns: self
    @objc open class func cycleImage(localizationImageNameArray: [String]?, frame: CGRect) -> LVCycleScrollView {
    ...
    ///   - frame: FWCycleScrollView的大小
    /// - Returns: self
    @objc open class func cycleImage(imageUrlStrArray: [String]?, placeholderImage: UIImage?, frame: CGRect) -> LVCycleScrollView {
    ...
    ///   - frame: FWCycleScrollView的大小
    /// - Returns: self
    @objc open class func cycleView(viewArray: [UIView]?, frame: CGRect) -> LVCycleScrollView {
...
extension LVCycleScrollView {
    ...
    public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if self.imageUrlStrArray != nil || self.localizationImageNameArray != nil {
            let cell = collectionView.dequeueReusableCell(withReuseIdentifier: kImageViewCellId, for: indexPath) as! LVUIImageViewCell
            ...
            let cell = collectionView.dequeueReusableCell(withReuseIdentifier: kViewCellId, for: indexPath) as! LVUIviewCell
...
extension LVCycleScrollView {
    ...
    private func setupPageControl() {
        ...
        case .custom:
            let tmpPageControl = LVPageControl()
            tmpPageControl.numberOfPages = self.sourceCount
...
extension LVCycleScrollView {
    ...
    public func setupTimer() {
        ...
        if self.autoScroll {
            ...
            RunLoop.main.add(self.rollTimer!, forMode:.common)
        }
    }
    ...
    public func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        if self.pageControl!.isKind(of: UIPageControl.self) {
            ...
            (self.pageControl as! LVPageControl).currentPage = indexOnPageControl
        }
    }
