...
# 【配置上传到蒲公英相关信息】(可选)
__PGYER_U_KEY="3dc3f0ff8ff75532263c6ad702565e60"
__PGYER_API_KEY="69d1d0ad1f7ad12b5ee7b43839e0ccb9"

# 【配置上传到 Fir】(可选)
__FIR_API_TOKEN="xKKdjdldlodeikK626266skdkkddK"

# 【配置证书】(如果只有一个证书时该项 可选)
__CODE_SIGN_DISTRIBUTION="Apple Distribution: GRAND CHANNEL GB ENTERTAINMENT LIMITED (6MM8Y2RAD2)Co., Ltd."
__CODE_SIGN_DEVELOPMENT="Apple Development: qichen jiang (88SCQB6C54)"

# 发布APP Store 账号密码
__IOS_SUBMIT_ACCOUNT="1286594907@qq.com"
__IOS_SUBMIT_PASSWORD="Wss103123.."
...
# 1. 请选择 SCHEME
__SELECT_TARGET_OPTIONS=("1.Rightly")
...
if [[ $__SELECT_TARGET_OPTION -eq 1 ]]; then
  __BUILD_TARGET="Rightly"
  __SCHEME_NAME="Rightly"
else
...
__BUNDLE_BUILD_VERSION = $(/usr/libexec/PlistBuddy -c "Print CFBundleVersion" "${INFOPLIST_FILE}")
...
if [[ $__IS_WORKSPACE_OPTION -eq 1 ]]; then
  ...
  if [[ ${__BUILD_CONFIGURATION} == "Debug" ]]; then
    # step 1. Clean
    xcodebuild clean  -workspace $(dirname $PWD)/${__PROJECT_NAME}.xcworkspace \
    -scheme ${__SCHEME_NAME} \
    -configuration ${__BUILD_CONFIGURATION}
    
    # step 2. Archive
    xcodebuild archive  -workspace $(dirname $PWD)/${__PROJECT_NAME}.xcworkspace \
    -scheme ${__SCHEME_NAME} \
    -configuration ${__BUILD_CONFIGURATION} \
